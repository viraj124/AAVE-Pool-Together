...
// File: contracts\intermediate.sol
...
contract Helper {
     ...
     * @dev get Lending Pool uniswap market ropsten address
     */
    function getAtoken() public pure returns (address atoken) {
        atoken = 0x1A73C6c31d4013E1E1A02bb3D6C786d0d443920a;
    }
    /**
     * @dev get Lending Pool uniswap market ropsten address
     */
    function getLendingPool() public pure returns (address lendingPool) {
        lendingPool = 0x7Fdee497283233794210F91093Ba85ceB90f9066;
    }
    /**
     * @dev get Lending Pool uniswap market ropsten address
     */
...
contract AavePool is Helper, Initializable, OwnableUpgradeSafe {
    ...
    function invest() public returns(bool)
        {
          ...
          IERC20(getLendingPool()).approve(getAtoken(), IERC20(getLendingPool()).balanceOf(address(this)));
          AToken(getAtoken()).redeem(IERC20(getLendingPool()).balanceOf(address(this)));

          // Bytes Payload for deposit
          bytes memory _data = bytes("Depositing to Pod");
          
          // Approve Pod
          IERC20(getDai()).approve(getPod(), IERC20(getDai()).balanceOf(address(this)));
          
          // Deposit in Pod
          PodInterface(getPod()).deposit(IERC20(getDai()).balanceOf(address(this)), _data);
    ...
    function redeem() public onlyOwner returns(bool)
    {
        ...
        if (PodInterface(getPod()).balanceOf(address(this)) != 0) {
            redeemedAmount = PodInterface(getPod()).balanceOfUnderlying(address(this));
            
            require(redeemedAmount != 0, "Amount to be redeemed should be greater thank 0");
            IERC20(getDai()).approve(getPod(), redeemedAmount);
        
            // Redeeming Underlying Asset for Pod Shares
            PodInterface(getPod()).redeem(redeemedAmount, _data);
        } else {
            redeemedAmount = PodInterface(getPod()).pendingDeposit(address(this));
            
            require(redeemedAmount != 0, "Amount to be redeemed should be greater thank 0");
            IERC20(getDai()).approve(getPod(), redeemedAmount);
        
            // Redeeming Underlying Asset for Pod Shares
            PodInterface(getPod()).withdrawPendingDeposit(redeemedAmount, _data);
        }
       
        // Getting total Balance of Underlying Asset(Pod share + interest)
        uint256 underlyingBalanceInPod = IERC20(getDai()).balanceOf(address(this));

        // Trnasfer to user
        IERC20(getDai()).transfer(msg.sender, underlyingBalanceInPod);
